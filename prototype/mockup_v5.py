...
# --- continent outlines (lon,lat), used as a land mask ---
CONTINENTS=[
 # North America
 [(-168,66),(-158,71),(-128,71),(-95,73),(-82,73),(-78,63),(-80,52),(-66,60),(-56,52),
  (-66,46),(-70,41),(-74,40),(-81,31),(-81,25),(-90,29),(-97,26),(-97,18),(-105,20),
  (-110,23),(-114,30),(-124,34),(-124,40),(-124,48),(-130,54),(-140,60),(-152,59),(-165,60)],
 # Central America bridge
 [(-92,18),(-83,15),(-77,8),(-82,8),(-90,13),(-94,16)],
 # South America
 [(-78,8),(-71,11),(-62,10),(-50,5),(-35,-5),(-35,-12),(-39,-18),(-48,-25),(-53,-34),
  (-58,-39),(-63,-42),(-66,-45),(-69,-52),(-74,-53),(-73,-44),(-71,-33),(-71,-20),
  (-76,-14),(-81,-6),(-80,2),(-78,8)],
 # Greenland
 [(-46,60),(-30,60),(-20,70),(-22,78),(-40,83),(-58,80),(-55,68)],
 # Europe
 [(-10,37),(-9,44),(-2,43),(-1,49),(-5,50),(2,51),(-4,58),(5,62),(10,64),(22,66),(30,62),
  (24,56),(30,52),(40,48),(28,44),(24,40),(15,40),(12,46),(6,44),(3,43),(-3,36)],
 # Scandinavia tip
 [(14,68),(20,70),(28,71),(24,64),(16,62)],
 # Africa
 [(-17,15),(-16,21),(-12,28),(-2,35),(10,37),(11,33),(20,33),(33,31),(34,28),(43,12),
  (51,11),(48,2),(41,-2),(40,-10),(35,-18),(32,-26),(27,-34),(20,-35),(18,-29),(13,-17),
  (9,-2),(8,4),(-2,5),(-8,4),(-13,9),(-17,15)],
 # Madagascar
 [(43,-13),(50,-15),(50,-25),(45,-25),(43,-18)],
 # Asia (incl. Middle East, India, Siberia, China, SE Asia)
 [(28,44),(40,48),(48,52),(58,52),(58,46),(50,42),(52,38),(48,30),(44,38),(36,36),(35,30),
  (43,24),(58,24),(60,25),(67,25),(67,37),(72,33),(70,28),(78,24),(80,14),(78,8),(85,20),
  (90,22),(98,16),(106,10),(106,21),(112,22),(122,40),(127,52),(135,55),(142,60),(160,62),
  (180,66),(178,70),(140,73),(100,77),(70,72),(58,70),(48,66),(40,66),(34,60),(30,52)],
 # Japan
 ...
 # Australia
 [(114,-22),(122,-18),(130,-12),(137,-12),(143,-11),(146,-18),(151,-24),(153,-31),
  (149,-38),(140,-38),(131,-32),(123,-34),(115,-34),(114,-28)],
]
...
def is_land(lon,lat):
    for poly in CONTINENTS:
        if _inpoly(lon,lat,poly): return True
    return False
# halo behind sphere + ocean disk
glow_disk=Image.new("RGB",(W,H),(0,0,0)); gdd=ImageDraw.Draw(glow_disk)
gdd.ellipse([gcx-R-6,gcy-R-6,gcx+R+6,gcy+R+6],fill=(14,20,28))
glow_disk=glow_disk.filter(ImageFilter.GaussianBlur(14)); img=ImageChops.screen(img,glow_disk); d=ImageDraw.Draw(img)
d.ellipse([gcx-R,gcy-R,gcx+R,gcy+R],fill=(11,14,19),outline=(40,48,60),width=1)
# dotted land only, depth-shaded
lat=-86
while lat<=86:
    cl=max(0.12,math.cos(math.radians(lat))); lon=-180; step=2.2/cl
    while lon<180:
        if is_land(lon,lat):
            X,Y,Z=sph(lat,lon)
            if Z>0.02:
                b=0.30+0.70*Z
                col=(int(48+150*b),int(78+150*b),int(96+150*b))
                r=1.0 if Z<0.55 else 1.5
                d.ellipse([X-r,Y-r,X+r,Y+r],fill=col)
        lon+=step
    lat+=2.2
# rim highlight
d.ellipse([gcx-R,gcy-R,gcx+R,gcy+R],outline=(58,68,82),width=1)
# arcs (thin amber/green, with end nodes)
hubs={"NY":(40,-74),"LDN":(51,0),"FRA":(50,8),"DXB":(25,55),"SAO":(-23,-46)}
pos={n:sph(la,lo) for n,(la,lo) in hubs.items()}
flows=[("LDN","NY",ACC),("DXB","LDN",GREEN),("SAO","NY",GREEN),("FRA","NY",ACC)]
for a,b,col in flows:
    ...
    tt=0.6; bx=(1-tt)**2*x1+2*(1-tt)*tt*mxp+tt*tt*x2; by=(1-tt)**2*y1+2*(1-tt)*tt*myp+tt*tt*y2
    d.ellipse([bx-2,by-2,bx+2,by+2],fill=WHITE)
for n,(X,Y,Z) in pos.items():
    if Z>0:
        d.ellipse([X-3,Y-3,X+3,Y+3],fill=ACC)
        d.ellipse([X-5,Y-5,X+5,Y+5],outline=ACC,width=1)
# corner stats
...
img.save("/home/user/tv_license_app/prototype/atlas_v5.png","PNG"); print("saved")
